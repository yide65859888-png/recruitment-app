...
TEAM_MEMBERS = ["储郑燃", "刘春雨", "唐凯", "孙衍", "王俊丽", "王小兰", "王肖", "王莉莹", "栗阳阳", "罗悦", "薛丽丽"]
...
def mock_employee_ocr(image):
    return {
        ...
        "exchanged_contact": 14, "exchanged_phone": 8, "proposed_interview": 5, 
...
def render_full_ranking(df, col_name, title_name, unit=""):
    ...
    for i in range(len(df_sorted)):
        ...
        rank_class = "rank-item-top1" if i == 0 else ("rank-item-top2" if i == 1 else ("rank-item-top3" if i == 2 else "rank-item-normal"))
        prefix = f"🥇 第1名" if i == 0 else (f"🥈 第2名" if i == 1 else (f"🥉 第3名" if i == 2 else f"&nbsp;&nbsp;&nbsp;第{i+1}名"))
...
if page == "📱 员工端：手机填报与截图上传":
    ...
    with st.form("employee_upload_form", clear_on_submit=True):
        ...
        record_date = st.date_input("数据日期", datetime.date(2026, 7, 21))
        platform_ver = st.selectbox("选择账号版本/平台", ["易德Boss", "呼叫中心Boss", "人保Boss", "智联招聘", "51job"])
    ...
    if submit_btn:
        if uploaded_file is not None:
            ...
            conn = sqlite3.connect('recruitment_data.db')
            c = conn.cursor()
            c.execute("INSERT INTO platform_data (date, employee_name, platform_version, seen_me, i_communicated, received_resumes, exchanged_contact, exchanged_phone, proposed_interview, accepted_interview) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
                      (record_date.strftime("%Y-%m-%d"), emp_name, platform_ver, ocr["seen_me"], ocr["i_communicated"], ocr["received_resumes"], ocr["exchanged_contact"], ocr["exchanged_phone"], ocr["proposed_interview"], ocr["accepted_interview"]))
            conn.commit()
            conn.close()
            st.balloons()
            st.success(f"🎉 提交成功！[{emp_name}] 的 [{platform_ver}] 数据已自动识图保存！")
        else:
    ...
    if "单月" in view_mode:
        ...
        selected_m_str = col_m.selectbox("月份", [f"{m:02d}月" for m in range(1, 13)], index=6)
        month_str = f"{selected_year}-{selected_m_str.replace('月', '')}"
        date_filter_p = f"{month_str}%"
        date_filter_perf = f"{month_str}%"
    else:
        selected_date = col_date.date_input("选择统计日期", datetime.date(2026, 7, 21))
        date_str = selected_date.strftime("%Y-%m-%d")
        date_filter_p = date_str
        date_filter_perf = date_str

    selected_employees = col_filter.multiselect("筛选员工姓名", TEAM_MEMBERS, default=TEAM_MEMBERS)
    
    conn = sqlite3.connect('recruitment_data.db')
    if "单月" in view_mode:
        df_p = pd.read_sql_query("SELECT employee_name as 员工姓名, SUM(seen_me) as 看过我, SUM(i_communicated) as 主动沟通, SUM(received_resumes) as 收获简历, SUM(exchanged_contact) as 交换微信, SUM(exchanged_phone) as 交换电话, SUM(proposed_interview) as 拟约面, SUM(accepted_interview) as 接受面试 FROM platform_data WHERE date LIKE ? GROUP BY employee_name", conn, params=(date_filter_p,))
        df_perf = pd.read_sql_query("SELECT employee_name as 员工姓名, MAX(month_invites) as 单月累计邀约, MAX(month_interviews) as 单月累计到面, MAX(month_trainees) as 单月累计参培 FROM performance_data WHERE date LIKE ? GROUP BY employee_name", conn, params=(date_filter_perf,))
    else:
        df_p = pd.read_sql_query("SELECT employee_name as 员工姓名, SUM(seen_me) as 看过我, SUM(i_communicated) as 主动沟通, SUM(received_resumes) as 收获简历, SUM(exchanged_contact) as 交换微信, SUM(exchanged_phone) as 交换电话, SUM(proposed_interview) as 拟约面, SUM(accepted_interview) as 接受面试 FROM platform_data WHERE date = ? GROUP BY employee_name", conn, params=(date_filter_p,))
        df_perf = pd.read_sql_query("SELECT employee_name as 员工姓名, SUM(invites) as 当日邀约, SUM(interviews) as 当日到面, SUM(trainees) as 当日参培 FROM performance_data WHERE date = ? GROUP BY employee_name", conn, params=(date_filter_perf,))
    conn.close()
    ...
    df_summary["到面转化率数值"] = df_summary.apply(
        lambda r: (r['单月累计到面']/r['单月累计邀约']*100) if "单月" in view_mode and r['单月累计邀约'] > 0 
        else ((r['当日到面']/r['当日邀约']*100) if r.get('当日邀约', 0) > 0 else 0.0), axis=1
    )
    df_summary["到面转化率"] = df_summary["到面转化率数值"].apply(lambda x: f"{x:.1f}%")

    if "单月" in view_mode:
        st.subheader(f"📆 招聘单月全链路累计汇总大表 ({month_str})")
        final_cols = ["员工姓名", "看过我", "主动沟通", "收获简历", "交换微信", "交换电话", "拟约面", "接受面试", "单月累计邀约", "单月累计到面", "单月累计参培", "到面转化率"]
    else:
        st.subheader(f"📋 招聘当日全链路整合数据一览表 ({date_str})")
        final_cols = ["员工姓名", "看过我", "主动沟通", "收获简历", "交换微信", "交换电话", "拟约面", "接受面试", "当日邀约", "当日到面", "当日参培", "到面转化率"]
    ...
    if alerts:
        for a in alerts:
            ...
            st.markdown(f"<div class='{card_class}'><b>【{a['level']}】{a['name']} - {a['issue']}</b><br/>• <b>卡点数据：</b> {a['data']}<br/>• <b>归因诊断：</b> {a['reason']}<br/>• <b>建议动作：</b> {a['action']}</div>", unsafe_allow_html=True)
    else:
        st.success("🎉 当前全链路数据表现顺畅，暂无卡点预警！")

elif page == "📋 数据端：智能识图/录入业绩":
    st.markdown("<div class='main-header'>📋 数据端：部门业绩汇总与智能识图录入</div>", unsafe_allow_html=True)
    col_type, col_date = st.columns(2)
    data_type = col_type.radio("📌 选择上传的数据表类型：", ["📅 日度新增业绩表", "📆 单月累计业绩表"], horizontal=True)
    p_date_img = col_date.date_input("选择业绩数据日期", datetime.date(2026, 7, 21))
    
    st.write("---")
    if "日度" in data_type:
        daily_img = st.file_uploader("上传【日度】表格截图", type=["jpg", "png", "jpeg"])
            ...
            if st.button("💾 确认提交【日度】数据入库"):
                ...
                for idx, row in edited_df.iterrows():
                    if row["员工姓名"] in TEAM_MEMBERS:
                        c.execute("INSERT INTO performance_data (date, employee_name, invites, interviews, trainees) VALUES (?, ?, ?, ?, ?)", (date_str, row["员工姓名"], int(row["当日邀约"]), int(row["当日到面"]), int(row["当日参培"])))
                conn.commit()
                conn.close()
                st.balloons()
                st.success(f"🎉 成功保存 {date_str} 数据！")
    else:
        monthly_img = st.file_uploader("上传【单月累计】表格截图", type=["jpg", "png", "jpeg"])
        if monthly_img is not None:
            ...
            if st.button("💾 确认提交【单月累计】数据入库"):
                ...
                for idx, row in edited_df.iterrows():
                    if row["员工姓名"] in TEAM_MEMBERS:
                        c.execute("INSERT INTO performance_data (date, employee_name, month_invites, month_interviews, month_trainees) VALUES (?, ?, ?, ?, ?)", (date_str, row["员工姓名"], int(row["单月累计邀约"]), int(row["单月累计到面"]), int(row["单月累计参培"])))
                conn.commit()
                conn.close()
                st.balloons()
                st.success(f"🎉 成功保存 {date_str} 数据！")